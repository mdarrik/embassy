...
impl<'d> Uart<'d, Blocking> {
    ...
    pub fn new_blocking<T: Instance>(
        ...
        into_ref!(tx, rx);
        Self::new_inner(T::info(), rx.map_into(), tx.map_into(), None, None, None, None, config)
    }
...
mod eh1 {
    ...
    impl<'d, M: Mode> embedded_hal_1::serial::nb::Read for UartRx<'d, M> {
    ...
    impl<'d, M: Mode> embedded_hal_1::serial::blocking::Write for UartTx<'d, M> {
    ...
    impl<'d, M: Mode> embedded_hal_1::serial::nb::Write for UartTx<'d, M> {
    ...
    impl<'d, M: Mode> embedded_hal_1::serial::nb::Read for Uart<'d, M> {
        fn read(&mut self) -> Result<u8, nb::Error<Self::Error>> {
            embedded_hal_02::serial::Read::read(&mut self.rx)
        }
    }

    impl<'d, M: Mode> embedded_hal_1::serial::blocking::Write for Uart<'d, M> {
        fn write(&mut self, buffer: &[u8]) -> Result<(), Self::Error> {
            self.blocking_write(buffer)
        }
    ...
    impl<'d, M: Mode> embedded_hal_1::serial::nb::Write for Uart<'d, M> {
        fn write(&mut self, char: u8) -> nb::Result<(), Self::Error> {
            self.blocking_write(&[char]).map_err(nb::Error::Other)
        }
